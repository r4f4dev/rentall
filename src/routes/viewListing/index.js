...
import {
  ...
  getListBlockedDates,
  getFullBlockDates,
} from "../../actions/Listing/getSpecialPricingData";
...
export default async function action({ params, store, query }) {
  ...
  if (data && data.getListMeta) {
    ...
    tariff = data.getListMeta.tariff;
    if (data.getListMeta.listPhotos && data.getListMeta.listPhotos.length > 0) {
      let coverImage = data.getListMeta.listPhotos.find(
        (o) => o.id == data.getListMeta.coverPhoto
      );
      listPhoto =
        url +
        fileuploadDir.replace(".", "") +
        (coverImage ? coverImage.name : data.getListMeta.listPhotos[0].name);
    }
  } else {
    renderNotFound();
    return;
  ...
  return {
    ...
    image: listPhoto || "",
    component: (
      <Layout viewListingHeader={viewListingHeader}>
        <ViewListing
          title={title}
          preview={preview}
          lat={lat}
