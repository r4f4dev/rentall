...
const csvRoutes = app => {
	...
	app.get('/export-transaction', async (req, res) => {
		var type = req.query && req.query.type,
			...
			searchKey = req.query && req.query.searchKey,
			siteName = req.query && req.query.siteName;
		...
		res.setHeader('Content-disposition', 'attachment; filename=' + siteName + '-' + type + '-transactions.csv');
		...
		try {
			...
			if (req.user && req.user.admin && type) {
				...
				if (type === 'users') {
					...
					data = await reservations(keyword, toCurrency);
				}
