...
class PayoutMethods extends Component {
  ...
  render() {
    ...
    return (
      <div className={cx('inputFocusColor', 'commonListingBg', 'noMarginBottom', 'payoutRadioBtn')}>
        <form onSubmit={handleSubmit}>
          ...
          {
            !loading && getPaymentMethods != undefined && getPaymentMethods.results && getPaymentMethods.results.length > 0 && <div className={cx('payoutTable', 'NewResponsiveTable', 'mobileTable')}>
              <table className={cx('table', s.noBorder)}>
                <thead>
                  <tr className={cx(s.rowBorder, s.sectionTitleLight, s.textTruncate)}>
                    ...
                    <th scope="col" className={s.noBorder}><FormattedMessage {...messages.payoutTitle1} /></th>
                    <th scope="col" className={s.noBorder}><FormattedMessage {...messages.payoutTitle2} /></th>
                    <th scope="col" className={s.noBorder}><FormattedMessage {...messages.payoutTitle3} /></th>
                      ...
                      if (item.isEnable) {
                        return (
                          <tr className={cx(s.sectionTitleLight)} key={index}>
                            <td data-label={formatMessage(messages.payoutTitle)}>
                              <Field
                                name="methodId"
                                component="input"
                                value={item.id}
