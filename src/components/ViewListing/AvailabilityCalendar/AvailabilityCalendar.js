...
class AvailabilityCalendar extends React.Component {
  ...
  componentDidMount() {
    ...
    if (queryStartDate && queryEndDate) this.setState({ startDate: moment(queryStartDate), endDate: moment(queryEndDate) });
  }
  ...
  async onDatesChange({ startDate, endDate }) {
    ...
    if (formName === 'PaymentForm' || formName === 'PaymentFormModal') {
      await change(formName, 'checkIn', startDate);
  ...
  render() {
    ...
    const { verySmallDevice, listId, paymentCalendar } = this.props;
