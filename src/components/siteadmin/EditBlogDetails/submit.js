...
async function submit(values, dispatch) {
  ...
  `;
    ...
    if (data.updateBlogDetails.status === "success") {
      toastr.success("Success!", "Content page has been updated successfully!");
      history.push('/siteadmin/content-management');
    }
    else if (data.updateBlogDetails.status === 'URL exist') {
      toastr.error("Failed!", "The page URL already exist!");
    }
    else {
      toastr.error("Failed!", "Updating location failed");
    }
  }
