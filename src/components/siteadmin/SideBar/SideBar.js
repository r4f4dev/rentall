...
import becomeHostIcon from '../../../../public/adminIcons/becomeHost.svg';
				...
				<div className={cx({ [s.menuOpen]: this.state.isOpen == 1 }, s.sidebarWrapper, 'adminScrollBar')}>
					...
					<div className={cx("hidden-print", s.sidebarNavParent)}>
						...
						<ul className={s.sidebarNav}>
							...
							}
							{
								...
								<li className={cx({ [s.active]: location === '/siteadmin/static-block' })}>
									<Link to={'/siteadmin/static-block'} className={cx(s.sideNavitem, s.disPlayTable)} onClick={() => this.openClose()}>
										<span className={s.disPlayTabelCell}>
											<img src={becomeHostIcon} className={s.navigationIcon} />
										</span>
							...
							{
								validatePrivilege(4, privileges) && <li className={cx({ [s.active]: (location === "/siteadmin/tariffs" || location.startsWith('/siteadmin/viewtariffs/')) })} >
									<Link to={'/siteadmin/tariffs'} className={cx(s.sideNavitem, s.disPlayTable)} onClick={() => this.openClose()}>
										<span className={s.disPlayTabelCell}>
											<img src={manageRevertionIcon} className={s.navigationIcon} />
										</span>
										{/* <span className={s.disPlayTabelCell}><FormattedMessage {...messages.manageReservations} /></span> */}
										<span className={s.disPlayTabelCell}>Тарифы</span>
									</Link>
								</li>
							}
						</ul>
